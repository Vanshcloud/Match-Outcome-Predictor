...
"AFC Bournemouth" here and "Bournemouth" in the ingested table are the same
club, and the difference is a convention rather than a name.
"""
...
def club_words(name: str) -> str:
    ...
    ``"Nott'm Forest"`` becomes ``"nottm forest"`` — the apostrophe closes up
    rather than splitting the word, because ``"nott m forest"`` would be three
    tokens where the club has two.

    **Accents are folded, and that is not a guess.** This feed writes "Grêmio",
    "São Paulo", "Alavés" and "1. FC Köln"; the ingested table writes the same
    clubs without the marks. Two spellings of one letter is an encoding
    convention, not a different name — folding them is worth 14 of the 164
    clubs this dashboard can be asked about, measured, and it is the only rule
    here that changes a letter rather than dropping one.
    """
    folded = unicodedata.normalize("NFKD", name.casefold())
    kept = "".join(
        " " if not letter.isalnum() and letter != "'" else letter
        for letter in folded
        if not unicodedata.combining(letter)
    ).replace("'", "")
    return " ".join(word for word in kept.split() if word not in SUFFIXES)
