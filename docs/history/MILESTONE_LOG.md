...
The full, per-milestone record of what changed and why, as it was written
during the build. It was the project's changelog until the public release;
[CHANGELOG.md](../../CHANGELOG.md) now summarises it. Milestone numbers match
[DEVELOPMENT_HISTORY.md](../DEVELOPMENT_HISTORY.md). Some entries describe
features later removed from the dashboard (the closing-line panel and squads,
Milestones 17 and 18).

## [Unreleased]

### Fixed

- **A refused fixture-feed request no longer reads as "no fixtures".** The
  forward sections checked the feed with one request and filled themselves
  from another; when the second failed (a 429 on the free tier, a timeout),
  Home and the competition pages said "No fixtures scheduled in the next week"
  or "Nothing in play right now". They now show the feed's reason.
- **The dashboard no longer hangs when the prediction service is down.** The
  client inherited the downloader's five retries with backoff, so each call to a
  service that was not running took 15 seconds. It now retries once, and the
  error is one short sentence instead of urllib3's connection-pool text.
- **The head-to-head table names competitions and formats dates** instead of
  showing `ENG_1` and `2026-09-06 00:00:00`, and a null score reads as a dash.
- **A played match the service has not indexed says why:** a match ingested
  after the feature table was built needs `make ratings features` and a restart.
- **Stale references to the removed market, odds and squad panels** are gone
  from the README, `docs/DASHBOARD.md`, `SECURITY.md`, module docstrings and
  `.env.example`, which also loses the unused `DASHBOARD_SQUAD_PROVIDER`. The
  README screenshots are recaptured from the current dashboard.
- **The archive sample size is stated with its power.** 2,286 scored forecasts
  is where a 0.0163 shift reaches a 95% interval's half-width, which detects it
  about half the time; 80% power needs about 4,670.
...
- `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, a bug report form and a pull request
  template.
...
- `scripts/explain.py` (`make explain`), `scripts/model_card.py` (`make card`),
  **[docs/EXPLAINABILITY.md](../EXPLAINABILITY.md)**, and a CI invariant
...
- `src/pipelines/train.py`, `scripts/train.py`, `make train` (~8 minutes) and
  `make ablation` (~5 minutes), plus **[docs/MODELS.md](../MODELS.md)**.
...
- `src/pipelines/backtest.py`, `scripts/backtest.py`, `make backtest` (~5
  seconds) and **[docs/EVALUATION.md](../EVALUATION.md)**.
...
- `tests/unit/test_leakage_suite.py` — both temporal probes over every
  ...
  hundred.
- `scripts/audit_columns.py`, `make audit`, and **[docs/LEAKAGE.md](../LEAKAGE.md)**:
